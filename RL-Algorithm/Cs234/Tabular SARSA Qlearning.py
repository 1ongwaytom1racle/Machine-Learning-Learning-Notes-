...
for i in range(4):
    ...
    while True:
        ...
        next_action = env.epsilon_greedy(next_state,i)
        
        sarsa = np.array([current_state,current_action,reward,next_state,next_action])
        memory.append(sarsa)
        
        env.q_table[current_state, current_action] += (
        env.alpha * (reward
        + env.q_table[next_state, next_action]
        - env.q_table[current_state, current_action]))

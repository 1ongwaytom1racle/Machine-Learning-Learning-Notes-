...
Dim = 2
#train_prex = np.concatenate((train_theta_scaled,train_cd_scaled),axis = 1)
#train_ATTX, _ = create_dataset(train_prex,Motion_history)
#train_ATTX = np.reshape(train_ATTX, (train_ATTX.shape[0], train_ATTX.shape[1], Dim))
...
Model = Sequential([
      #Conv1D(1, 1, strides=1, padding='valid', data_format='channels_last', dilation_rate=1),#, activation='relu'),
      LSTM(4, input_shape=(Motion_history, 1), activation='tanh',return_sequences=True),
      Attention(name='attention_weight'),
      #Conv1D(1, 1, strides=1, padding='valid', data_format='channels_last', dilation_rate=1),
      #LSTM(4, input_shape=(Motion_history, 1), activation='tanh',return_sequences=False),
      Dense(1)
])

Model.compile(loss='mse', optimizer='adam')
history = Model.fit(train_THETA, train_CIP, epochs=1000, verbose=0)
print(Model.summary())
Model.save_weights('Model/Cip_LSTM_time_att_4_1000.h5')
...
Model.load_weights('Model/time_att_4_1000.h5')
"""
"""""""""""""""""""""""""""""""""""""""
训练集查看
"""""""""""""""""""""""""""""""""""""""
...
plt.show()
train_score = math.sqrt(mean_squared_error(train_CIP[0], train_predict_cip[:,0]))
...
plt.setp(ltext, fontsize=20)
plt.show()
test_score = math.sqrt(mean_squared_error(test_cipdata[Motion_history + 1:],test_predict[:]))
